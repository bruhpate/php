...
if (isset($_SESSION["UTENTE"])) {
    ...
    if (count($_POST)>0){
        ...
        if($_POST["tipo"] == "Aggiungi"){
            $sql = "INSERT INTO utente (nome) VALUES ('$_POST[nome]')";
        ...
        if($_POST["tipo"] == "Modifica"){
            $sql = "UPDATE utente SET nome='$_POST[nome]' WHERE Id=$_POST[id]";
            if ($conn->query($sql) === TRUE) {
                echo "New record created successfully";
                header("Refresh:0");
              } else {
        ...
        if($_POST["tipo"] == "Elimina"){
            $sql = "DELETE FROM utente WHERE Id=$_POST[id]";
            if ($conn->query($sql) === TRUE) {
                echo "New record created successfully";
                header("Refresh:0");
              } else {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<p>Utente</p>";
        echo "<br><table style='border:1px solid'><t><th>Id</th><th>Nome</th></tr>";
